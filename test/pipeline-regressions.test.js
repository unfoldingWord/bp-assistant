...
test('runMechanicalQualityPrep re-sequences the notes file by ULT-alignment position when aligned USFM is available', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'notes-quality-prep-seq-'));
  const oldBaseDir = process.env.CSKILLBP_DIR;
  process.env.CSKILLBP_DIR = tempDir;

  // CSKILLBP_DIR is a load-time constant captured independently by both
  // pipeline-utils and tsv-tools, so clear both caches.
  const notesPipelinePath = require.resolve('../src/notes-pipeline');
  const qualityToolsPath = require.resolve('../src/workspace-tools/quality-tools');
  const utilsPath = require.resolve('../src/pipeline-utils');
  const tsvToolsPath = require.resolve('../src/workspace-tools/tsv-tools');
  delete require.cache[notesPipelinePath];
  delete require.cache[qualityToolsPath];
  delete require.cache[utilsPath];
  delete require.cache[tsvToolsPath];
  const { _runMechanicalQualityPrep: runMechanicalQualityPrep } = require('../src/notes-pipeline');

  try {
    const pipeDir = path.join('tmp', 'pipeline', 'GEN-001');
    const pipeAbs = path.join(tempDir, pipeDir);
    fs.mkdirSync(pipeAbs, { recursive: true });

    const notesRel = path.join('output', 'notes', 'GEN', 'GEN-001.tsv');
    const alignedRel = path.join(pipeDir, 'ult-aligned.usfm');

    fs.mkdirSync(path.join(tempDir, 'output', 'notes', 'GEN'), { recursive: true });
    // "late" quote (בָּרָ֣א, position 2) written before "early" quote
    // (בְּרֵאשִׁ֖ית, position 1) — out of alignment order on disk.
    fs.writeFileSync(path.join(tempDir, notesRel), [
      'Reference\tID\tTags\tSupportReference\tQuote\tOccurrence\tNote',
      '1:1\tlate1\t\t\tבָּרָ֣א\t1\tLate note',
      '1:1\tearly1\t\t\tבְּרֵאשִׁ֖ית\t1\tEarly note',
    ].join('\n'));

    fs.writeFileSync(path.join(tempDir, alignedRel), [
      '\\id GEN',
      '\\c 1',
      '\\v 1 \\zaln-s |x-content="בְּרֵאשִׁ֖ית"\\*\\w In|x\\w* \\zaln-s |x-content="בָּרָ֣א"\\*\\w created|x\\w*',
    ].join('\n'));

    fs.writeFileSync(path.join(pipeAbs, 'context.json'), JSON.stringify({
      book: 'GEN',
      sources: { ultAligned: alignedRel },
      runtime: {},
    }, null, 2));

    await runMechanicalQualityPrep({ notesPath: notesRel, pipeDir, status: async () => {}, ref: 'GEN 1' });

    const lines = fs.readFileSync(path.join(tempDir, notesRel), 'utf8').trim().split('\n');
    const ids = lines.slice(1).map((line) => line.split('\t')[1]);
    assert.deepEqual(ids, ['early1', 'late1']);
  } finally {
    if (oldBaseDir == null) delete process.env.CSKILLBP_DIR;
    else process.env.CSKILLBP_DIR = oldBaseDir;
    delete require.cache[notesPipelinePath];
