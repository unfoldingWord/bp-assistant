...
const { orchestrations } = require('./orchestrations/index');
...
Options:
  --provider <name>     Provider: gemini, openai, xai, claude (default: gemini)
...
async function main() {
  ...
  // Validate provider
  const validProviders = ['gemini', 'openai', 'xai', 'claude'];
  ...
  // Check API key
  const keyEnvMap = { gemini: 'GEMINI_API_KEY', openai: 'OPENAI_API_KEY', xai: 'XAI_API_KEY', claude: 'ANTHROPIC_API_KEY' };
  const envKey = keyEnvMap[args.provider];
  if (!process.env[envKey] && !args.dryRun) {
    console.error(`Error: ${envKey} not set. Export it or add to /srv/bot/config/.env`);
    process.exit(1);
  }
