...
function sweepWorkspaceOwnership({ baseDir = CSKILLBP_DIR, subdirs = ['output', 'tmp'], log = console } = {}) {
  // The expected owner is whoever owns the workspace root (the bot user). Fall
  // back to skipping if the workspace is absent (local dev / unit tests).
  let targetUid;
  let targetGid;
  try {
    const rootStat = fs.statSync(baseDir);
    targetUid = rootStat.uid;
    targetGid = rootStat.gid;
  } catch (_) {
    return null;
  }
  ...
  const visit = (p) => {
    ...
    if (st.uid !== targetUid || st.gid !== targetGid) {
      foreign.push(p);
      if (canChown) {
        try { fs.chownSync(p, targetUid, targetGid); chowned++; } catch (_) { /* best effort */ }
      }
    }
    if (st.isDirectory()) {
      let entries = [];
  ...
  if (foreign.length) {
    ...
    const action = canChown
      ...
      : ' — running unprivileged, could not chown (these may cause EACCES)';
    log.warn(
      `[ownership-sweep] ${foreign.length} path(s) under ${baseDir} not owned by uid ${targetUid}:${targetGid}${action}\n  ` +
      preview.join('\n  ') + more,
    );
  }

  return { foreign, chowned, canChown, targetUid, targetGid };
}
...
module.exports = { sweepWorkspaceOwnership, sweepStaleTmp };
